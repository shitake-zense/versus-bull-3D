...
`CameraController` wraps drei `OrbitControls` (pan disabled, distance/polar clamped) and has 3 view presets (keys 1/2/3) lerped over 600ms via a shared `beginAnim(toPos, toTarget)` helper + a `useFrame` eased lerp. **Follow-cam** (HUD「🎥 追従」toggle, `localStorage: vsb3.followCam`, default OFF): when on, each change of `followTarget` (= the view's `lastMove`, so it also follows replay steps) runs `framePose(cell, layer)` and lerps the camera to orbit onto the **same side as the moved cell** (cell in the foreground, unoccluded) with the target biased toward it — reusing the exact same anim path as presets. Manual drag still works; the next move re-frames. The background space color is `#1C4A6E` (deep blue, so white-O/black-X both pop).
